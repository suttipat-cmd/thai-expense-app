...
- เพิ่ม loading/disabled state ในจุดสำคัญ เช่น โหลดข้อมูล บันทึก แก้ไข ลบ และ OCR สลิป
- เพิ่ม OCR สลิปธนาคารด้วย Typhoon API
- OCR สลิปจะตั้งเป็น `รายจ่าย` เสมอ
- OCR เป็นแค่ตัวช่วยกรอกฟอร์ม ผู้ใช้ต้องตรวจสอบก่อนกดบันทึก
- ไม่เก็บภาพสลิปลง Google Sheet
...
- ใส่ Typhoon API Key ไว้ใน `Code.gs` เท่านั้น ไม่ใส่ใน frontend
...
- แก้ไขรายการเดิม
- ลบรายการเดิม
- สแกนสลิปธนาคารเพื่อกรอกข้อมูลรายจ่าย
...
- ถ้ายังไม่ใส่ Apps Script URL ใน `script.js` จะใช้งานด้วยข้อมูลในเครื่องผ่าน `localStorage` ได้ก่อน แต่จะยัง OCR สลิปไม่ได้
...
- รายการล่าสุด
- ปุ่มโหลดข้อมูลใหม่
- loading ตอนดึงข้อมูลจาก Google Sheet
...
```text
หน้าเว็บ -> ลดขนาดรูปในเครื่อง -> ส่งรูปเป็น base64 ไป Apps Script -> Apps Script เรียก Typhoon OCR API -> คืนผล OCR -> เติมฟอร์ม -> ผู้ใช้ตรวจสอบ -> กดบันทึกเอง
```

### สิ่งที่ OCR พยายามดึง

- ยอดเงิน
- วันที่
- ชื่อร้านหรือผู้รับเงิน ถ้าอ่านได้
- ข้อความ OCR บางส่วนเพื่อใส่ในหมายเหตุ

### ข้อกำหนดสำคัญ

- สลิปที่ OCR ได้จะเป็น `รายจ่าย` เสมอ
- หมวดหมู่เริ่มต้นเป็น `อื่นๆ`
- รูปสลิปไม่ถูกบันทึกลง Google Sheet
- รูปสลิปไม่ถูกแนบไปกับรายการ
- OCR อาจอ่านผิดได้ ผู้ใช้ต้องตรวจสอบก่อนกดบันทึก
- รองรับรูป PNG/JPEG เป็นหลัก
...
สไตล์ทั้งหมดของเว็บ ใช้แนว mobile-first และออกแบบให้เหมือน mobile app รวมถึง loading, OCR card และ disabled state

### script.js

ตรรกะทั้งหมดของเว็บ เช่น การเรนเดอร์หน้าจอ การจัดการฟอร์ม การคำนวณสรุป การบันทึกในเครื่อง การเชื่อมต่อ Apps Script และการส่งรูปสลิปไป OCR
...
const TYPHOON_API_KEY = "PASTE_TYPHOON_API_KEY_HERE";
```
...
รูปสลิปจะไม่ถูกบันทึกใน Google Sheet
...
6. แก้ค่า `TYPHOON_API_KEY` ด้านบนสุดของ `Code.gs`

ตัวอย่าง:

```javascript
const TOKEN = "my-secret-token-123";
const TYPHOON_API_KEY = "typhoon-api-key-ของคุณ";
```
...
4. ใส่จำนวนเงิน หรือกด `เลือกรูปสลิป` เพื่อใช้ OCR
5. เลือกหมวดหมู่
6. เลือกวันที่
...
1. เปิดหน้า `เพิ่มรายการ`
2. กด `เลือกรูปสลิป`
3. เลือกภาพสลิปหรือถ่ายภาพจากมือถือ
4. รอระบบอ่านสลิป
5. ตรวจสอบจำนวนเงิน วันที่ และหมายเหตุ
6. เลือกหมวดหมู่ที่ต้องการ
7. กด `บันทึก`

หมายเหตุ: OCR จะไม่บันทึกรายการให้อัตโนมัติ เพื่อให้ผู้ใช้ตรวจยอดก่อนเสมอ
...
| delete | ลบรายการ |
| ocrSlip | รับภาพสลิป ส่งต่อ Typhoon OCR และคืนข้อมูลสำหรับเติมฟอร์ม |

เว็บเรียก action ทั่วไปด้วย JSONP เพื่อให้ใช้งานจาก GitHub Pages ได้ง่ายและลดปัญหา CORS

สำหรับ `ocrSlip` เว็บจะส่งภาพผ่าน hidden form + iframe bridge เพราะ payload รูปภาพมีขนาดใหญ่เกินกว่าจะส่งผ่าน JSONP URL ได้
...
## การตั้งค่า Typhoon API Key

ให้ใส่ Typhoon API Key ใน `Code.gs` เท่านั้น

```javascript
const TYPHOON_API_KEY = "typhoon-api-key-ของคุณ";
```

ไม่ควรใส่ Typhoon API Key ใน `script.js` เพราะ `script.js` อยู่ฝั่ง frontend และผู้ใช้สามารถดูโค้ดได้

หลังแก้ `TYPHOON_API_KEY` ต้อง Deploy Apps Script ใหม่
...
- ไม่มีแนบรูปสลิป
- รูปสลิปไม่ถูกเก็บ แต่ถูกส่งไป Typhoon API เพื่อประมวลผล OCR
- OCR อาจอ่านยอดเงินหรือวันที่ผิด ต้องตรวจสอบก่อนกดบันทึก
...
- ปรับ parser OCR ตามตัวอย่างสลิปจริงที่ปิดข้อมูลส่วนตัวแล้ว
- ค้นหารายการ
- กรองตามหมวดหมู่
- Export CSV
- ตั้งงบรายเดือน
...
- เพิ่ม OCR สลิปธนาคารด้วย Typhoon API
- OCR เติมข้อมูลเป็นรายจ่ายเสมอ
- เพิ่มปุ่ม `เลือกรูปสลิป` ในหน้าเพิ่มรายการ
- เพิ่ม loading/disabled state สำหรับโหลดข้อมูล บันทึก ลบ และ OCR
- เพิ่ม hidden form + iframe bridge สำหรับส่งรูปไป Apps Script โดยไม่ใช้ CORS
- Apps Script ไม่เก็บรูปสลิปลง Google Sheet
- อัปเดต README ให้เป็นข้อมูลล่าสุดครบชุด
